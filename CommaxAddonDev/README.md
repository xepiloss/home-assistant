...
- 실외 미세먼지 확인
- 예보 날씨와 최고/최저 온도 확인
- TCP 수신 중 패킷 경계가 밀리거나 일부가 씹혀도 가능한 경우 정상 상태 프레임을 복구해 반영
...
생활정보 날씨 센서는 실제 월패드 표시값과 대조해 확인된 값만 Home Assistant에 등록합니다.
...
수집 파일에는 아직 파서가 없는 프레임과 프레이밍 중 버려진 바이트가 고유 패킷별로 한 줄씩 저장됩니다. 같은 `source`, `kind`, `hex` 조합의 패킷은 새 줄을 만들지 않고 `count`, `first_seen`, `last_seen`, 반복 간격 요약(`repeat_interval_*`)과 최근 수신 시각 일부(`recent_seen_at`)만 갱신합니다.
