...
수집 파일에는 아직 파서가 없는 프레임, 프레이밍 중 버려진 바이트, 생활정보 `8F` 프레임, 생활정보 `24 01` 온도 프레임이 고유 패킷별로 한 줄씩 저장됩니다. 같은 `source`, `kind`, `hex` 조합의 패킷은 새 줄을 만들지 않고 `count`, `first_seen`, `last_seen`, `seen_at` 수신 시각 배열만 갱신합니다. 미세먼지, 날씨, 기온처럼 생활정보 표시값이 바뀌면 `8F` 또는 `24 01` 패킷의 `hex`도 달라질 수 있어 실제 표시값과 수신 시간을 비교해 매핑할 수 있습니다.
